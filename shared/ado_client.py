...
class ADOClient:
    ...
    def get_wiki_page(self, wiki_id: str, path: str) -> dict[str, Any]:
        """Fetch an existing Wiki page, injecting the ETag from the response header."""
        url = (
        ...
        resp.raise_for_status()
        data = resp.json()
        # ADO returns the ETag in the response header, not the body
    ...
    def upsert_wiki_page(
        ...
        try:
            ...
            if status == 500 and not extra_headers.get("If-Match"):
                # ADO returns 500 when updating an existing page without an ETag.
                # Re-fetch the current ETag and retry with it.
                current_etag = self.get_wiki_page_version(wiki_id, path)
                if current_etag:
                    extra_headers["If-Match"] = current_etag
                    result = self._put(url, {"content": content}, extra_headers=extra_headers, params=params)
                else:
                    # Page doesn't exist yet — ensure ancestors and create
